...
def process_log(filename, outfile):
    ...
    P.plot_date(t01, means[0] * y01, 'b:', label='Average (%.0f Mbit/s : %.0f%%)' %
                (means[0], means[0] / maxes[0] * 100))
    P.plot_date(t, v[:, 1] / 1e6, 'r-', label='Upload')
    P.plot_date(t01, means[1] * y01, 'r:', label='Average (%.0f Mbit/s : %.0f%%)' %
                (means[1], means[1] / maxes[1] * 100))
